 ...
 *
 */
public class Findfilmcompany extends Activity {
	...
	Handler  handler = new Handler(){
		public void handleMessage(android.os.Message msg) {
			switch(msg.what){
			case 1:
				CompanyDetailAdapter sa =new CompanyDetailAdapter(Findfilmcompany.this, lists);
	...
	private void initThread() {
		...
		new  Thread(new Runnable(){	
			@Override
			public void run() {
				if(NetWorkUtil.isNetAvailable(Findfilmcompany.this)){
					try {
						ss = new HttpP().sendPOSTRequestHttpClient(url, paramsValue);
						if(ss==null){
							Toast.makeText(Findfilmcompany.this, "正在加载", 1000).show();
						}
						lists =	getJson(ss);
					...
					Toast.makeText(Findfilmcompany.this, "连接网络", 1000).show();
				}
			}
		}).start();;
	...
	private void initView() {
		...
		sm.pushActivity(Findfilmcompany.this);
	...
	OnItemClickListener ocl =new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				long arg3) {
			Intent it0= new Intent(Findfilmcompany.this,FindFouractivity.class);
